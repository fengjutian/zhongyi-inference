...
import type { RuleFormEmits, RuleFormData } from '@/types/components'
...
// 计算属性
const isEditMode = computed(() => !route.meta.create_flag)
const formTitle = computed(() => route.meta.create_flag ? '创建规则' : '编辑规则')

// 初始化表单数据
const initFormData = async () => {  
  if (route.meta.create_flag) {
    // 新增
    ...
    // 编辑
    // 根据params的id去请求对应的rule
    const ruleId = Number.parseInt(route.params.id as string)
...
const handleSubmit = async () => {
  try {
    ...
    if (isEditMode.value && formData?.id) {
      ...
      await ruleService.updateRule(formData.id, submitData)
    } else {
      // 新增
      await ruleService.createRule(submitData)
    }
    router.back()
...
const readonly = computed(() => !route.meta.create_flag);
const mode = computed(() => route.meta.create_flag);
